    ...
    def get_icd_map(self):
        ...
        with open(self.f_icdmap, "r") as f:
            ...
            for line in reader:
                ...
                self.condensed_icd_to_desc[line[1]] = line[3]
        ...
        with open(self.f_notes, "r") as f:
            ...
            for line in reader:
                ...
                if category == "Discharge summary" and hadmid in self.hadmid_to_icds and len(
                    ...
                    start_phrase = "Brief Hospital Course:"
                    end_phrase = "Medications on Admission:"
                    start_idx = text.find(start_phrase)
                    end_idx = text.find(end_phrase)

                    if start_idx == -1:
                        start_phrase = "HOSPITAL COURSE:"
                        start_idx = text.find(start_phrase)
                        if start_idx > -1:
                            end_idx = len(text)
            ...
            with open(self.f_output + "/icd.processed.txt", "w") as icd_fw:
                ...
                with open(self.f_output + "/icd-desc.processed.txt", "w") as icd_desc_fw:
                    ...
                    for hadmid, text in hadmid_to_note.items():
                        ...
                        if len(patient_icds) == 0 or len(text) == 0:
                            continue
