...
class BertClassifierWithMetadataXS(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.bert = BertModelWithMetadata(config)
        self.preclassifier1 = nn.Linear(config.hidden_size, config.hidden_size)
        self.preclassifier2 = nn.Linear(config.hidden_size, config.hidden_size)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(config.hidden_size, config.num_labels)
        self.init_weights()
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, metadata_input_ids=None, metadata_len=None):
        ...
        pooled_output = self.preclassifier1(pooled_output)
        pooled_output = self.preclassifier2(pooled_output)
