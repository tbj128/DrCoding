...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, metadata_hidden_states=None):
        mixed_query_layer = self.query(hidden_states) # bs, seq len, embed size
        if metadata_hidden_states is not None:
            mixed_key_layer = self.key(metadata_hidden_states) # bs, metadata seq len, embed size
        else:
            mixed_key_layer = self.key(hidden_states) # bs, seq len, embed size
        mixed_value_layer = self.value(hidden_states) # bs, seq len, embed size
